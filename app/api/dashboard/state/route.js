...
export async function GET(request) {
  ...
  const leads = await sql`
    ...
    WHERE threads_user_id=${owner.userId}
      ...
      AND (${search}='' OR body ILIKE ${`%${search}%`} OR username ILIKE ${`%${search}%`})
  ...
  return NextResponse.json({ settings: settingRows[0], leads, runs, stats: statRows[0], account: owner.account }, { headers: { "Cache-Control": "no-store" } });
}
