...
本機 Codex 排程每小時執行一次：先透過受 HMAC 密鑰保護的端點觸發 Threads 蒐集，再下載候選資料、完成語意篩選並加密回傳。Vercel Hobby 方案不允許小時級 Cron，因此 `vercel.json` 的每日工作只保留為備援；每小時主排程需要本機與 Codex 可執行。同一小時的重跑會防重，超過設定蒐集天數的資料會在工作中清理。
