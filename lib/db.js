...
async function createSchema() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS collector_settings (
      ...
      collection_days INTEGER NOT NULL DEFAULT 7 CHECK (collection_days BETWEEN 1 AND 30),
      schedule TEXT NOT NULL DEFAULT '08:30',
      timezone TEXT NOT NULL DEFAULT 'Asia/Taipei',
      tone TEXT NOT NULL DEFAULT '專業親切',
      offer TEXT NOT NULL DEFAULT '提供快速回覆與一對一需求評估',
      ai_provider TEXT NOT NULL DEFAULT 'openai' CHECK (ai_provider IN ('openai','local_codex')),
  ...
  await sql`CREATE INDEX IF NOT EXISTS collection_run_items_recorded_idx ON collection_run_items(recorded_at)`;
  await sql`
  ...
  await sql`ALTER TABLE collector_settings ADD COLUMN IF NOT EXISTS ai_provider TEXT NOT NULL DEFAULT 'openai'`;
  await sql`ALTER TABLE collector_settings ADD COLUMN IF NOT EXISTS collection_days INTEGER NOT NULL DEFAULT 7`;
  ...
  await sql`
    UPDATE collector_settings AS s SET ai_provider='local_codex'
    FROM threads_accounts AS a
    WHERE s.threads_user_id=a.threads_user_id AND a.role='owner'`;
