...
export async function collectAccount(userId, { triggerType = "manual" } = {}) {
  ...
  const runRows = await sql`
    ...
    WHERE collection_runs.status IN ('failed','skipped')
       OR (collection_runs.status='running' AND collection_runs.started_at < NOW() - INTERVAL '5 minutes')
    RETURNING id`;
  ...
  try {
    ...
    const collectionBudgetMs = triggerType === "local_hourly" ? 38_000 : 48_000;
    ...
    while (items.size < fetchTarget && states.some(state => !state.done) && Date.now() - started < collectionBudgetMs) {
      for (const state of states) {
        if (state.done || items.size >= fetchTarget || Date.now() - started >= collectionBudgetMs) continue;
        try {
    ...
    if (rows.length) {
      const inserted = await sql`
        ...
        ON CONFLICT DO NOTHING RETURNING id, threads_id`;
      insertedCount = inserted.length;
      if (inserted.length) {
        const runItems = inserted.map(item => ({ lead_id: String(item.id) }));
        await sql`
          INSERT INTO collection_run_items (run_id, lead_id)
          SELECT ${runId}, x.lead_id::bigint
          FROM jsonb_to_recordset(${JSON.stringify(runItems)}::jsonb) AS x(lead_id text)
