...
export async function collectAccount(userId, { triggerType = "manual" } = {}) {
  ...
  const idempotencyKey = triggerType === "cron" ? `daily:${userId}:${taipeiDate()}` : null;
  const runRows = await sql`
    ...
    RETURNING id`;
  if (!runRows.length) return { skipped: true, reason: "今天的自動蒐集已執行" };
  ...
  try {
    ...
    await sql`DELETE FROM leads WHERE threads_user_id=${userId} AND published_at < NOW() - INTERVAL '7 days'`;
    ...
    while (items.size < target && states.some(state => !state.done) && Date.now() - started < 48_000) {
      for (const state of states) {
        if (state.done || items.size >= target || Date.now() - started >= 48_000) continue;
        try {
    ...
    if (rows.length) {
      const inserted = await sql`
        ...
        SELECT ${userId}, x.threads_id, x.username, x.body, x.published_at::timestamptz, x.permalink,
          ...
          x.demand_level, x.demand_reason, x.suggested_copy
        FROM jsonb_to_recordset(${JSON.stringify(rows)}::jsonb) AS x(
          ...
          demand_level text, demand_reason text, suggested_copy text)
        ON CONFLICT DO NOTHING RETURNING id`;
      insertedCount = inserted.length;
    }
    const details = { keywordErrors: states.filter(state => state.error).map(state => ({ keyword: state.keyword, error: state.error })), exhausted: states.every(state => state.done) };
    await sql`UPDATE collection_runs SET status='complete', raw_count=${rawCount}, inserted_count=${insertedCount}, duplicate_count=${duplicateCount}, too_old_count=${tooOldCount}, details=${JSON.stringify(details)}::jsonb, finished_at=NOW() WHERE id=${runId}`;
    return { runId, target, rawCount, insertedCount, duplicateCount, tooOldCount, shortfall: Math.max(0, target - insertedCount), details };
  } catch (error) {
