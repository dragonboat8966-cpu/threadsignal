...
export async function screenPendingLeads(userId, { limit = MAX_PER_REQUEST } = {}) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    return {
      ...
      pendingCount: Number((await sql`SELECT COUNT(*)::int count FROM leads WHERE threads_user_id=${userId} AND classification_source='pending' AND published_at >= NOW() - INTERVAL '7 days'`)[0]?.count || 0)
    };
  }
