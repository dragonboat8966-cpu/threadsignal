...
export async function requireAdminOwner() {
  const workspace = await requireWorkspaceUser();
  return workspace?.account?.role === "owner" ? workspace : null;
}

// Backward-compatible name while dashboard routes migrate from the former
// single-owner authorization model.
export const requireOwner = requireWorkspaceUser;
